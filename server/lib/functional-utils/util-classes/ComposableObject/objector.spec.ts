...
describe('ComposableObject Entries', () => {
  ...
  it('should return a new object from original object properties', () => {
    ...
    const result: object = ComposableObject.entries(fn(obj), obj)
      .reduce((acc: object, item: any) => {
        return {
          ...acc,
          ...item,
        };
  ...
  it('should allow devs to utilize a pipe composition', () => {
    ...
    const reducto = (results: object[]): object => results
      .reduce((acc: object, item: object) => {
        return {
          ...acc,
          ...item,
        };
    ...
    const result = (obj): object => pipe(
      ComposableObject.entries(fn(obj)),
      reducto,
    )(obj);
  ...
  it('should return a new object from original object properties', () => {
    ...
    const fn = curry((obj, property): object => {
      return {
        ...
        [`${property}Length`]: obj[property].length,
      };
    });

    const result: object = ComposableObject.keys(fn(obj), obj)
      .reduce((acc: object, item: any) => {
        return {
          ...acc,
          ...item,
...
describe('ComposableObject Values', () => {
  ...
  it('should allow devs to use pipe composition', () => {
    ...
    const result = (object: Record<string, any>): unknown[] => pipe(
      objValues
    )(object);
...
describe('ComposableObject From Entries Create Object', () => {
  ...
  it('should throw an error when not given a proper tuple', () => {
    ...
    const expectation = {
      Error: 'Iterator value prop1 is not an entry object',
    };

    const errorResult: Record<string, any> = pipe(
      ComposableObject.fromEntries
    )(objEntries);

    const errorResult2: Record<string, any> = pipe(
      ComposableObject.fromEntries
    )('boo');
