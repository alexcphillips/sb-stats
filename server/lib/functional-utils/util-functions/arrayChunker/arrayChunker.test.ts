...
describe('arrayChunker', () => {
  it('correctly splits array into chunks of a specified size', () => {
    ...
    expect(arrayChunker(3, arrayOfSix)).toEqual([[1, 2, 3], [4, 5, 6]]);
    expect(arrayChunker(4, arrayOfEight)).toEqual([[1, 2, '3', 4], [5, {a: 1}, 7, 8]]);
    expect(arrayChunker(5, arrayOfEight)).toEqual([[1, 2, '3', 4, 5], [{a: 1}, 7, 8]]);
  });
