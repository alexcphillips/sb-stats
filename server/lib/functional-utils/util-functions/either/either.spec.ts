...
const either2 = Identity(3).map(either(is3, multiplyBy11, not3Error)).chain(either(isNumber, multiplyBy11, notNumberError));
const either3 = Identity(3).map(either(is3, multiplyBy11, not3Error)).map(multiplyBy11).chain(add2);
const eitherFail = Identity(4).chain(either(is3, multiplyBy11, not3Error));
const eitherFail2 = Identity(3).map(either(is3, multiplyBy11, not3Error)).map((x) => `I am a String ${x}`).chain(either(isNumber, multiplyBy11, notNumberError));
...
describe('Either Function', () => {
  ...
  describe('checker as boolean', () => {
    it('should return value', () => {
      ...
      expect(either(negitiveCheckerFn, doTrue, doFalse)(truthyValue)).toEqual('didFalse' + truthyValue);
      expect(either(negitiveCheckerFn, doTrue, doFalse)(falseyValue)).toEqual('didTrue' + falseyValue);
    });
  });
});
