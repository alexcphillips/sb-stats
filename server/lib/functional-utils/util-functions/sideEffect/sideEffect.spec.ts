...
type MockSideEffectFn = (arg: ObjectType) => string
const mockSideEffectFn: MockSideEffectFn = jest.fn(({ value }) => value);
const mockSideEffectFnFail: MockSideEffectFn = jest.fn(() => { throw Error('fail'); });

type MockAsyncSideEffectFn = (arg: ObjectType) => Promise<string>
const mockAsyncSideEffectFn: MockAsyncSideEffectFn = jest.fn(async ({ value }) => Promise.resolve(value));
