...
const Either1 = ErrorMonad(is3, multiplyBy11, not3Error, 3)
  .join();

const Either2 = ErrorMonad(is3, multiplyBy11, not3Error, 3)
  .chain(ErrorMonad(isNumber, multiplyBy11, notNumberError)).join();

const Either3 = ErrorMonad(is3, multiplyBy11, not3Error, 3)
  .map(multiplyBy11)
  .chain(add2);

const EitherFail = ErrorMonad(is3, multiplyBy11, not3Error, 4)
  .join();
