...
describe('Maybe', () => {
  ...
  describe('map', () => {
    it('should apply function(s) to value', () => {
      ...
      expect(Maybe(onFail, isNotNaN, 0).map(x => x + 1).join()).toEqual(1);
      expect(Maybe(onFail, isNotNaN, 0).map(x => x + 1).map(x => x + 1).join()).toEqual(2);
    });
  ...
  describe('chain', () => {
    it('should apply function(s) and return the value', () => {
      ...
      expect(Maybe(onFail, isNotNaN, 0).map(x => x + 1).chain(x => x + 1)).toEqual(2);
    });

    it('should not apply function when value is nothing', () => {
      const isNotNaNOrNull = (x) => !isNaN(x) && x !== null;
      expect(Maybe(onFail, Boolean, undefined).chain(x => x + 1)).toEqual('I failed with value: undefined');
      expect(Maybe(onFail, Boolean, null).chain(x => x + 1)).toEqual('I failed with value: null');
      expect(Maybe(onFail, isNotNaNOrNull, 0).map(() => null).chain(x => x + 1)).toEqual('I failed with value: null');
    });
  });
});
