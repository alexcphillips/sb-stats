...
const result1 = Either(Identity, ShortCircuit, evaluation(3))
  .map(add1)
  .chain(add2);

const result2 = Either(Identity, StringToIntConverter, evaluation('3'))
  .map(add1)
  .chain(add2);

const result3 = EitherLeft.of(3)
  .map(add1)
  .chain(add2);

const result4 = EitherRight.of(3)
  .map(add1)
  .chain(add2);

const fail1 = Either(Identity, ShortCircuit, evaluation('nope'))
  .map(add1)
  .chain(add2);

const fail2 = Either(Identity, StringToIntConverter, evaluation('apple'))
  .map(add1)
  .chain(add2);

const fail3 = Either(Identity, StringToIntConverter, shortedEval('apple'))
  .map(add1)
  .chain(add2);
